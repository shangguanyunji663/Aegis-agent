...
4. 在学生端顶栏点「主题切换」下拉，依次切到「深海冥想 / 晨雾森林 / 童趣治愈贴贴 / 暖意疗愈」，展示整页即时换色、刷新后仍是新主题（按账号持久化）。
5. 退出后使用 `admin / admin123!` 登录，进入管理员端。
6. 展示「风险报告」「个案」「知识库」「对话回放」与「工作台」（工具任务/执行记录/评测/审计页签）等管理模块；管理端顶栏同样有「主题切换」下拉，上次在学生端选的主题已同步到管理端（跨页面/跨设备同步）。
