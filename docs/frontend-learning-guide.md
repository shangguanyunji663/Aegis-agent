...
后端 `app/api/pages.py` 只有 26 行:三个路由(`/`、`/student`、`/admin`)各自 `read_text()` 读一个 HTML 文件原样返回,**没有模板引擎、没有字符串拼接、没有注入**。CSS/JS 由 `app/main.py` 的 `app.mount("/static", StaticFiles(...))` 作为静态文件服务。
...
| 文件 | 行数 | 职责 | 谁在用 |
| --- | --- | --- | --- |
| `styles.css` | ~1270 | 三页共用的全部样式:token 层 → 组件层 → 页面层 → 响应式 → 动效 | 三页 |
