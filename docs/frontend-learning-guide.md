...
| `index.html` + `login.js` | 77+97 | 登录/注册、角色重定向、服务状态胶囊 | 所有人 |
| `student.html` + `student.js` | 79+211 | 学生对话:SSE 流式渲染、会话历史、快捷表达 | 学生 |
| `admin.html` + `admin.js` | 141+283 | 教师后台:三列页签卡、列表渲染、审批动作、详情检查器 | 教师/管理员 |
...
3. **缓存指纹**:HTML 里引用资源带 `?v=0.8.0` 这样的版本号。改了 CSS/JS 就必须升级指纹,否则老访客的浏览器会用缓存里的旧文件——「我明明改了为什么没生效」九成是这个。
...
页面加载即调 `/api/health`,把「UP」翻译成「运行正常」、失败显示「服务异常」。这遵循全项目铁律:**状态展示要本地化、失败要给兜底文案,而不是白屏**。
...
历史列表点击 `loadConversation(id)` 拉全量消息重绘;`data-quick` 按钮把预设语句填入输入框(只填不发,把决定权留给学生——这是心理产品的细节);输入框 Enter 发送、Shift+Enter 换行(`requestSubmit()` 触发表单校验,比直接调 sendMessage 更正规)。
