...
>
> 读者定位:有 Python 基础、**不需要**前端框架经验的学生。你将看到:不用 React/Vue、不装 node\_modules、不打包,只用原生 HTML/CSS/JavaScript,也能做出体验完整的流式对话界面——并理解每一步「为什么这样做」。
>
...
## 1.1 渲染链路:浏览器拿到的就是仓库里的文件(第十八轮起多一段服务端主题注入)

后端 `app/api/pages.py` 约 60 行:三个路由(`/`、`/student`、`/admin`)各自 `read_text()` 读一个 HTML 文件返回,**没有模板引擎、没有字符串拼接**。唯一一处"注入"是第十八轮为主题切换加的:在 `<head>` 最前插入一段内联脚本,把当前登录用户的主题偏好写进 `html[data-theme]`,该脚本先于 `styles.css` 解析执行,首屏即为目标主题(详见 2.1 节)。CSS/JS 由 `app/main.py` 的 `app.mount("/static", StaticFiles(...))` 作为静态文件服务。

为什么这样设计:前端即静态文件,意味着 (1) 后端零渲染心智负担(主题注入是 6 行字符串替换,不算模板引擎),前后端只靠 JSON API 通信;(2) 部署时这 6 个文件可以独立放到任何 CDN;(3) 学习者看到的就是浏览器运行的,没有编译产物。

## 1.2 为什么零构建(不用 React/Vue/打包器)

- **教学优先**:克隆 → `pip install` → 打开浏览器,三步跑通;没有 `npm install` 失败、node 版本、打包配置这些「还没开始学就先劝退」的环节。

- **规模匹配**:三页界面、每页一个 JS 文件(119\~355 行),原生 JS 完全可控;框架的价值(组件化、响应式)在这个规模体现不出来。
...
| 文件                            | 行数      | 职责                                                                    | 谁在用               |
| ----------------------------- | ------- | --------------------------------------------------------------------- | ----------------- |
| `styles.css`                  | \~760   | 三页共用的全部样式:token 层(四套主题 CSS 变量) → 组件层 → 页面层 → 主题切换器 → 响应式 → 动效         | 三页                |
| `theme.js`                    | \~130   | 主题切换器:挂载 `#theme-switcher`、渲染菜单、即时切换 `html[data-theme]` 并 `PUT` 后端持久化 | 学生端/管理端顶栏(登录页不挂载) |
| `index.html` + `login.js`     | 121+119 | 登录/注册、角色重定向、Hero 区与状态胶囊(时段问候/服务状态)                                    | 所有人               |
| `student.html` + `student.js` | 112+234 | 学生对话(与小暖):SSE 流式渲染、消息头像与入场动画、话题 chips、心情速选、「需要立即帮助」求助卡                | 学生                |
...
# 第二部分 styles.css:四套疗愈主题的 CSS 变量设计系统

## 2.1 为什么先讲 CSS 变量(Token 层)——以及它如何支撑多主题切换

文件开头是 `:root` 里的几十个自定义属性(默认主题 `warm`):
...
为什么:全站 20+ 处用到主色,如果写死色值,「换个主题色」要改 20 处;用变量后只改 `:root` 一处。

第十八轮把第十五轮预留的"深色主题变量已备好但无开关"扩展点改造为**四套疗愈主题**:
`warm`(暖意疗愈,默认)/ `ocean`(深海冥想)/ `forest`(晨雾森林)/ `playful`(童趣治愈贴贴)。
每套主题对应一组 `html[data-theme="..."] { --bg: ...; --calm: ...; ... }` 变量块,
**组件层规则一行都不用改**,只靠属性选择器整体换值即可换色。主题键的单一真相源是
`app/repository/store.py` 的 `THEME_CHOICES`,新增主题只需三步:①在 `THEME_CHOICES`
追加键;②在 `styles.css` 新增对应 `html[data-theme="..."]` 变量块;③在 `theme.js` 的
`THEMES` 数组追加展示元数据。

`color-mix(in srgb, var(--calm) 30%, transparent)` 是"在 sRGB 色彩空间把两个颜色按比例混合"
——比手调透明度rgba更语义化,改 `--calm` 时这些派生色自动跟着变。

**服务端首屏注入(零闪烁)**:`app/api/pages.py` 在 `<head>` 最前注入一段内联脚本:

```html
<head><script>document.documentElement.setAttribute("data-theme", "ocean");</script>
```

该脚本**先于** **`styles.css`** **解析执行**,浏览器一开始就用目标主题的变量值布局,不出现
"先加载默认主题再跳变"的闪烁。未登录/无偏好记录时回退 `warm`。

**动手试一试**(无需改代码):登录后顶栏点主题切换器 → 选「深海冥想」→ 整页瞬间换色;
刷新页面仍是深海冥想(`PUT /api/auth/me/theme` 已写库);退出再登录仍是深海冥想(跨设备
同步)。也可直接 F12 → Console 输入:

```js
document.documentElement.setAttribute("data-theme", "playful")
```

回车,整页瞬间切到童趣治愈(薰衣草雾底 + 长春花紫 + 圆角更大 + 头像微倾斜)。刷新即恢复
为服务端注入的偏好主题。
...
# 第三部分 登录页:index.html + login.js(119 行的完整认证流)
...
批准/驳回/确认接案/重试按钮都是 `row()` 动态创建的,逐个绑事件要管理生命周期;本项目用**事件委托**:后台按钮带 `data-action`,在 `document.body` 上统一监听,点按后读 `target.dataset` 分发;学生端的 `data-quick`(心情速选卡与欢迎屏话题 chips)同理挂在 `document` 上。新增一种行内按钮只需在分发函数加一个 `if` 分支。
...
- **401 被踢回首页?** `api()` 约定:未登录统一跳转,是特性。`theme.js` 的 `PUT` 收到 401 也会跳回登录页。

- **SSE 只收到一半?** 看后端日志;前端 parseSse 对半截事件是安全的。

- **主题怎么切换/切换后没生效?** 学生端或管理端顶栏点「主题」下拉,选其中一套即可;切换后刷新仍是新主题(已写库)。没生效通常是 CSS/JS 指纹没升——`?v=0.14.1` 已升,清缓存或硬刷新(Ctrl+F5)。登录页无切换器(无登录用户),首屏仍由服务端注入默认主题 `warm`。

- **下拉菜单被对话区挡住?** 顶栏 `.role-topbar` 的 `z-index: 20` 高于 `.student-layout` / `.admin-layout` 的 1,菜单整体浮在对话/工作台之上;若仍被挡,检查是否改过顶栏 z-index 或对话区 `overflow: auto` 创建了新的层叠上下文。
